...
class Controller_Node(Node):
    ...
    def scan_callback(self, scan_msg):
        ...
        self.filtered_scan.clear() # remove previous values from sensor_msgs.msg import LaserScan
        ...
        if any(sector_value < 0.3 for sector_value in self.filtered_scan):
            self.near_wall = True
    ...
    def get_min_distance(self, scan_msg, min_index, max_index, switch_lectures):
        ...
        normalized_lectures = [scan_msg.range_max if x==float('inf') else x for x in section_lectures] # remove inf values
